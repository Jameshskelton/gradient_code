...
export const AVAILABLE_GRADIENT_MODELS: GradientModelOption[] = [
...
const MODEL_CAPABILITIES: Record<string, ModelCapabilityProfile> = {
  "glm-5": {
    model: "glm-5",
...
export function normalizeProviderModelName(model: string): string {
...
function withCapabilityDefaults(profile: ModelCapabilityProfile): ModelCapabilityProfile {
  const toolPromptStyle =
    profile.toolPromptStyle ??
    (profile.supportsToolCalling ? (profile.preferredEndpoint === "responses" ? "compact" : "minimal") : "none");

  return {
    ...profile,
    preferredStreamingEndpoint:
      profile.preferredStreamingEndpoint ??
...
function buildProviderError(statusCode: number, responseBody: string, model: string): Error {
  const error = statusCode === 404 && responseBody.includes("model not found")
    ? new Error(
        `Gradient model "${model}" was not found. This usually means the selected model ID is not in the current DigitalOcean catalog or is unavailable for this account.`,
      )
...
function isResponsesEndpointMismatch(error: unknown): boolean {
  ...
  return (
    message.includes("not supported in the v1/responses endpoint") ||
...
export function resolveModelCapabilityProfile(model: string): ModelCapabilityProfile {
...
function toMessages(systemPrompt: string, messages: AgentMessage[]): ChatCompletionMessage[] {
  ...
  for (const message of messages) {
    ...
    if (message.role === "assistant") {
      result.push({
        role: "assistant",
        content: message.content,
        tool_calls: message.toolCalls?.map((toolCall: ToolInvocation) => ({
          id: toolCall.callId,
          type: "function",
          function: {
            name: toolCall.name,
...
function summarizeSchemaParameters(schema: JsonSchema): string {
  if (!schema.properties) {
    return "";
  }
...
function buildCapabilityToolPrompt(profile: ModelCapabilityProfile, tools: ToolDefinition[]): string {
...
function adaptToolsForProfile(profile: ModelCapabilityProfile, tools: ToolDefinition[]): ToolDefinition[] {
...
function buildResponsesInput(messages: AgentMessage[]): ResponsesInputItem[] {
...
function parseTokenUsage(usage: any): TokenUsage | undefined {
...
function extractStreamingUsage(rawEvents: unknown[]): TokenUsage | undefined {
  for (let index = rawEvents.length - 1; index >= 0; index -= 1) {
    const event = rawEvents[index] as any;
    const usage =
      parseTokenUsage(event?.usage) ??
...
function mergeTextFragments(existing: string, incoming: string): string {
...
function reconcileStreamingSnapshot(
  existing: string,
  incoming: string,
...
function extractResponseTextPart(part: any): string {
...
function extractResponseItemText(item: any): string {
  ...
  if (Array.isArray(item.content)) {
    return item.content
      .map((part: any) => extractResponseTextPart(part))
...
function extractTopLevelResponseText(payload: any): string {
...
function parseResponsesTurn(payload: any): ProviderTurn {
  const outputs: ProviderTurn["outputs"] = [];
  let text = "";
...
function parseTurnResponse(payload: any): ProviderTurn {
  const outputs: ProviderTurn["outputs"] = [];
  ...
  const text = outputs
    .filter(
      (output: ProviderTurn["outputs"][number]): output is Extract<ProviderTurn["outputs"][number], { type: "text" }> =>
        output.type === "text",
    )
    .map((output: Extract<ProviderTurn["outputs"][number], { type: "text" }>) => output.text)
    .join("\n")
    .trim();
...
type ChatStreamingAccumulator = {
  id?: string;
  content: string;
  toolCalls: Map<number, { callId?: string; name?: string; arguments: string }>;
};

type ResponsesStreamingAccumulator = {
  id?: string;
  content: string;
...
function parseChatStreamingDelta(payload: any, accumulator: ChatStreamingAccumulator, handlers: ProviderStreamHandlers): void {
...
function finalizeChatStreamingTurn(accumulator: ChatStreamingAccumulator, rawEvents: unknown[]): ProviderTurn {
  const outputs: ProviderTurn["outputs"] = [];
...
function responsesToolCallKey(payload: any): string {
  return String(payload?.item_id ?? payload?.item?.id ?? payload?.call_id ?? payload?.output_index ?? "0");
}

function ensureResponsesToolCall(
  accumulator: ResponsesStreamingAccumulator,
  key: string,
...
function parseResponsesStreamingEvent(
  payload: any,
  accumulator: ResponsesStreamingAccumulator,
  handlers: ProviderStreamHandlers,
): void {
...
function finalizeResponsesStreamingTurn(accumulator: ResponsesStreamingAccumulator, rawEvents: unknown[]): ProviderTurn {
...
function parseSseEvent(record: string): SseEvent | null {
...
function requestSseWithNodeHttp(
  urlText: string,
  body: string,
  headers: Record<string, string>,
  timeoutMs: number,
...
function prepareProviderRequest(request: ProviderRequest): PreparedProviderRequest {
...
function parseEndpointTurn(endpoint: ProviderEndpoint, payload: any): ProviderTurn {
  return endpoint === "responses" ? parseResponsesTurn(payload) : parseTurnResponse(payload);
}

function preferredStreamEndpoint(profile: ModelCapabilityProfile): ProviderEndpoint | null {
...
export class GradientChatCompletionsClient implements ProviderClient {
  ...
  private endpointUrl(endpoint: ProviderEndpoint): string {
  ...
  private async fetchWithRetry(
    ...
    for (let attempt = 0; attempt <= this.retryCount; attempt += 1) {
      this.onDebug?.(
        `provider:attempt ${attempt + 1}/${this.retryCount + 1} endpoint=${endpoint} stream=false model=${request.model} timeoutMs=${String(timeoutMs)} url=${url}`,
      );
      ...
      try {
        ...
        if (response.statusCode >= 200 && response.statusCode < 300) {
          this.onDebug?.(
            `provider:response ok status=${response.statusCode} endpoint=${endpoint} stream=false attempt=${attempt + 1}`,
          );
          return response;
        }
        ...
        this.onDebug?.(
          `provider:response error status=${response.statusCode} endpoint=${endpoint} retryable=${String(retryable)} attempt=${attempt + 1} body=${responseBody.slice(0, 240)}`,
        );
  ...
  private async streamWithRetry(
    ...
    for (let attempt = 0; attempt <= this.retryCount; attempt += 1) {
      ...
      const chatAccumulator: ChatStreamingAccumulator = {
        content: "",
        toolCalls: new Map(),
      };
      const responsesAccumulator: ResponsesStreamingAccumulator = {
        content: "",
        toolCalls: new Map(),
      };
      ...
      this.onDebug?.(
        `provider:attempt ${attempt + 1}/${this.retryCount + 1} endpoint=${endpoint} stream=true model=${request.model} timeoutMs=${String(timeoutMs)} url=${url}`,
      );

      try {
        await requestSseWithNodeHttp(
          url,
          requestBody,
          requestHeaders,
          timeoutMs,
        ...
        this.onDebug?.(
          `provider:response ok status=200 endpoint=${endpoint} stream=true attempt=${attempt + 1}`,
        );
        ...
        this.onDebug?.(
          `provider:stream error endpoint=${endpoint} attempt=${attempt + 1} retryable=${String(retryable)} message=${lastError.message}`,
        );
  ...
  private async createTurnWithEndpoint(
    request: ProviderRequest,
    prepared: PreparedProviderRequest,
    endpoint: ProviderEndpoint,
  ): Promise<ProviderTurn> {
  ...
  async createTurn(request: ProviderRequest): Promise<ProviderTurn> {
  ...
  async createTurnStream(request: ProviderRequest, handlers: ProviderStreamHandlers): Promise<ProviderTurn> {
    const prepared = prepareProviderRequest(request);
    const streamEndpoint = preferredStreamEndpoint(prepared.profile);

    if (!streamEndpoint) {
      this.onDebug?.(
        `provider:stream disabled for model ${request.model}, using non-streaming ${prepared.profile.preferredEndpoint} turn`,
      );
    ...
    try {
      ...
      this.onDebug?.(
        `provider:stream failed for ${request.model}, falling back to non-streaming${fallbackToResponses ? " responses" : fallbackToChat ? " chat/completions" : ""}`,
      );
