...
function parseArgs(argv: string[]): CliOptions {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (part === "--preset") {
      options.preset = (argv[index + 1] as PromptPreset | undefined) ?? "default";
      index += 1;
      continue;
    }
...
async function promptForInput(label: string): Promise<string> {
...
function resolveConfig(cliOptions: CliOptions, fileConfig: GradientCodeConfig): ResolvedCliConfig {
  return {
...
function formatInput(inputArgs: Record<string, unknown>): string {
...
function renderSessionSummary(events: SessionEvent[], finalText: string): void {
  console.log("");
  console.log("Session Summary");
  console.log("===============");

  const userEvent = events.find((event): event is Extract<SessionEvent, { type: "user" }> => event.type === "user");
  if (userEvent) {
...
function renderToolIntent(toolCall: ToolInvocation): void {
  console.log("");
  console.log(`Tool> ${toolCall.name} ${formatInput(toolCall.arguments)}`);
}

function renderToolResult(toolCall: ToolInvocation, result: ToolResult): void {
  console.log(`Tool< ${toolCall.name} ${summarizeToolResult(result)}`);
}

function startAssistantStream(): { onDelta: (chunk: string) => void; complete: () => void } {
  let started = false;
  return {
...
): Promise<Pick<AgentSessionState, "id" | "messages" | "events" | "systemPrompt"> | null> {
...
async function main(): Promise<void> {
  ...
  if (!prompt) {
    throw new Error(
      "Usage: gradient_code [--model MODEL] [--cwd PATH] [--approve-all] [--no-store] [--no-preview-writes] [--session ID] [--resume-last] [--max-turns N] \"your prompt\"",
    );
  }
...
function timestampFromEvents(events: SessionEvent[]): string {
