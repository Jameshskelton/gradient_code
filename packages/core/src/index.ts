...
type KnownWorkspaceRegistry = {
  workspaces: string[];
  legacyDiscoveryCompleted: boolean;
};
...
function globalGradientCodeDir(): string {
...
function buildTurnSystemPrompt(
  ...
  if (options.turnIndex === 0) {
    reminders.push(
      "Turn-specific reminder: if you are about to use tools, first give the user a short explanation of what you are checking and why.",
    );
  }

  if (options.afterToolResults) {
    reminders.push(
      "Turn-specific reminder: start this turn with a brief progress update about what you learned from the last tool results and what you will do next before making more tool calls.",
    );
  }
...
async function readSessionState(filePath: string): Promise<AgentSessionState | null> {
  try {
    const raw = await fs.readFile(filePath, "utf8");
...
async function writeSessionState(filePath: string, state: AgentSessionState): Promise<void> {
  await fs.mkdir(path.dirname(filePath), { recursive: true });
  await fs.writeFile(filePath, `${JSON.stringify(state, null, 2)}\n`, "utf8");
}

function summarizeSession(state: AgentSessionState): SessionSummary {
...
function compareIsoTimestampDescending(left: string, right: string): number {
...
async function listSessionStatesInDirectory(directory: string): Promise<AgentSessionState[]> {
  try {
    ...
    return states.filter((state): state is AgentSessionState => Boolean(state));
  } catch {
    return [];
  }
}

async function readKnownWorkspaceRegistry(): Promise<KnownWorkspaceRegistry> {
  try {
    const raw = await fs.readFile(knownWorkspacesPath(), "utf8");
    const parsed = JSON.parse(raw) as Partial<KnownWorkspaceRegistry>;
...
async function writeKnownWorkspaceRegistry(registry: KnownWorkspaceRegistry): Promise<void> {
  await fs.mkdir(globalGradientCodeDir(), { recursive: true });
  await fs.writeFile(
    knownWorkspacesPath(),
    `${JSON.stringify(
      {
        workspaces: [...new Set(registry.workspaces.map((workspace) => normalizeWorkspacePath(workspace)))].sort((left, right) =>
          left.localeCompare(right),
        ),
        legacyDiscoveryCompleted: registry.legacyDiscoveryCompleted === true,
      } satisfies KnownWorkspaceRegistry,
      null,
      2,
    )}\n`,
    "utf8",
...
async function directoryHasSessionFiles(directory: string): Promise<boolean> {
  try {
    const entries = await fs.readdir(directory);
...
async function discoverLegacyWorkspaces(rootDir: string): Promise<string[]> {
...
async function ensureKnownWorkspaceRegistry(cwd?: string): Promise<{ workspaces: string[]; discoveredNow: boolean }> {
  ...
  const nextRegistry: KnownWorkspaceRegistry = {
  ...
  if (
    nextRegistry.legacyDiscoveryCompleted !== registry.legacyDiscoveryCompleted ||
...
async function syncWorkspaceSessionsToGlobal(cwd: string): Promise<void> {
  ...
  await Promise.all(
    states.map(async (state) => {
      const normalizedState: AgentSessionState = {
        ...state,
...
async function ensureGlobalSessionStore(cwd?: string): Promise<void> {
...
export async function deleteSessionsForWorkspace(cwd: string): Promise<number> {
  const resolvedCwd = normalizeWorkspacePath(cwd);
  const states = await listSessionStatesInDirectory(globalSessionsDir());
  ...
  await Promise.all(
    matching.map((state) =>
      fs.unlink(sessionFilePath(globalSessionsDir(), state.id)).catch(() => undefined),
    ),
  );
