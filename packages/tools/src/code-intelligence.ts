...
type DeclarationRecord = CodeSymbolMatch & {
...
function previewForPosition(sourceFile: ts.SourceFile, position: number): string {
...
function nodeNameText(name: ts.Node | undefined): string | null {
  if (!name) {
    return null;
  }
...
function declarationKindFromNode(node: ts.Node): CodeSymbolKind | null {
...
function containerNameForNode(node: ts.Node): string | undefined {
...
function collectDeclarations(sourceFile: ts.SourceFile): DeclarationRecord[] {
  const results: DeclarationRecord[] = [];

  function push(node: ts.Node, nameNode: ts.Node): void {
...
function normalizeFilterKind(kind?: string): CodeSymbolKind | null {
  if (!kind) {
    return null;
  }

  const normalized = kind.trim().toLowerCase();
  const kinds: CodeSymbolKind[] = ["function", "class", "interface", "type", "enum", "variable", "method", "property", "namespace"];
  return kinds.includes(normalized as CodeSymbolKind) ? (normalized as CodeSymbolKind) : null;
}

async function collectSourceFiles(root: string): Promise<{ fileNames: string[]; skippedLargeFiles: number }> {
...
async function resolveWorkspaceFiles(root: string): Promise<{
  fileNames: string[];
...
export async function createCodeWorkspace(root: string): Promise<CodeWorkspace> {
  const resolvedRoot = path.resolve(root);
...
function allDeclarations(workspace: CodeWorkspace): DeclarationRecord[] {
  const results: DeclarationRecord[] = [];
...
export function findReferences(
...
): { declaration: CodeSymbolMatch; references: CodeReference[] } | null {
  ...
  const groups = workspace.languageService.findReferences(
    declaration.filePath,
    declaration.nameNode.getStart(declaration.sourceFile),
  ) ?? [];
...
export function listExports(workspace: CodeWorkspace, filePath: string): CodeExport[] {
  const sourceFile = workspace.getSourceFile(filePath);
  if (!sourceFile) {
    return [];
  }

  const results: CodeExport[] = [];
...
export function listImports(workspace: CodeWorkspace, filePath: string): CodeImport[] {
  const sourceFile = workspace.getSourceFile(filePath);
  if (!sourceFile) {
    return [];
  }
...
function applyTextEdits(content: string, edits: Array<{ start: number; end: number; newText: string }>): string {
...
function parseNamedImports(values: string[]): Array<{ name: string; alias?: string }> {
  ...
  const unique = new Map<string, { name: string; alias?: string }>();
...
function readImportBindings(declaration: ts.ImportDeclaration): ImportBindings {
...
function mergeImportBindings(existing: ImportBindings, requested: ImportBindings): ImportBindings | null {
...
function buildRenameEdits(
  workspace: CodeWorkspace,
  declaration: DeclarationRecord,
  newName: string,
): FileChange[] {
  const renameLocations = workspace.languageService.findRenameLocations(
    declaration.filePath,
    declaration.nameNode.getStart(declaration.sourceFile),
    false,
    false,
...
function importModuleDeclarations(sourceFile: ts.SourceFile, moduleSpecifier: string): ts.ImportDeclaration[] {
...
function buildAddImportChange(
  ...
  const requestedBindings: ImportBindings = {
...
function buildRemoveImportChange(
  workspace: CodeWorkspace,
  filePath: string,
  request: AstEditRequest,
): FileChange[] {
  ...
  for (const declaration of declarations) {
    ...
    const nextBindings: ImportBindings = {
      ...
      namedImports: bindings.namedImports.filter(
        (entry) => entry.name !== bindingName && entry.alias !== bindingName,
      ),
      typeOnly: bindings.typeOnly,
    };

    const removedSomething =
      nextBindings.defaultImport !== bindings.defaultImport ||
...
function buildReplaceSymbolChange(
  workspace: CodeWorkspace,
  filePath: string,
  request: AstEditRequest,
): FileChange[] {
...
export function performAstEdit(workspace: CodeWorkspace, request: AstEditRequest): AstEditResult {
