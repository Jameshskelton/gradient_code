...
import type { Tool, ToolContext, ToolResult } from "@gradient-code/shared";
import {
  createCodeWorkspace,
  findReferences,
  findSymbols,
...
function ok(toolName: string, content: string, metadata?: Record<string, unknown>): ToolResult {
  return { ok: true, toolName, content, metadata };
}

function fail(toolName: string, code: string, message: string, metadata?: Record<string, unknown>): ToolResult {
...
function resolveWorkspacePath(cwd: string, target: string): string {
...
function relativeToWorkspace(cwd: string, target: string): string {
...
async function searchTextFallback(cwd: string, query: string): Promise<ToolResult> {
...
async function ensureCommandsDir(cwd: string): Promise<string> {
...
async function fetchTextFromUrl(url: string, timeoutMs = 15_000): Promise<string> {
...
function extractSearchResultsFromHtml(html: string, limit: number): Array<{ title: string; url: string }> {
...
function isBlockedNetworkTarget(urlText: string): string | null {
  let url: URL;
  try {
    url = new URL(urlText);
  } catch {
...
function sessionMetadataPath(logPath: string): string {
  return logPath.replace(/\.log$/, ".json");
}

async function persistCommandSession(session: CommandSession): Promise<void> {
  await fs.writeFile(
    sessionMetadataPath(session.logPath),
    `${JSON.stringify(
      {
        id: session.id,
...
async function appendCommandLog(session: CommandSession, chunk: string, stream: "stdout" | "stderr"): Promise<void> {
  await fs.appendFile(session.logPath, `[${stream}] ${chunk}`, "utf8");
}

async function createCommandSession(cwd: string, command: string, sessionId?: string): Promise<CommandSession> {
...
function getCommandSession(sessionId: string): CommandSession | undefined {
...
function renderCombinedDiff(cwd: string, changes: FileChange[]): string {
  return changes
...
async function writeTextFile(
  ...
  try {
    ...
    return writeTextFiles(
      context,
      toolName,
      [{ filePath, before: existing, after: content }],
      summaryPrefix,
    );
  } catch (error) {
...
async function writeTextFiles(
  ...
  const approved = await approveEdit(
    ...
    normalizedChanges.length === 1
      ...
      : `${summary} ${normalizedChanges.length} files`,
    diff,
  );
  ...
  const rollbackState = await Promise.all(
  ...
  const onDiskChanges: FileChange[] = [];
  for (const change of normalizedChanges) {
  ...
  const contentMismatch = onDiskChanges.find((change) => normalizeText(change.after) !== normalizeText(
    normalizedChanges.find((entry) => entry.filePath === change.filePath)?.after ?? "",
  ));
...
function formatSymbolMatch(cwd: string, match: CodeSymbolMatch): string {
...
function formatReference(cwd: string, reference: CodeReference): string {
...
function formatExport(cwd: string, filePath: string, item: CodeExport): string {
...
function formatImport(cwd: string, filePath: string, item: CodeImport): string {
...
function parseGitRenamePath(rawPath: string): { path: string; previousPath?: string } {
...
function mapGitStatusCode(code: string): string {
  switch (code) {
    case "M":
      return "modified";
    case "A":
...
function describeGitStatus(stagedCode: string, unstagedCode: string): string {
...
function parseGitStatusOutput(output: string): GitChangedFile[] {
...
function mergeNumstatEntry(
  map: Map<string, { additions?: number; deletions?: number; binary?: boolean; previousPath?: string }>,
  rawPath: string,
  additionsRaw: string,
  deletionsRaw: string,
): void {
...
function parseGitNumstat(output: string): Map<string, { additions?: number; deletions?: number; binary?: boolean; previousPath?: string }> {
  const map = new Map<string, { additions?: number; deletions?: number; binary?: boolean; previousPath?: string }>();
...
async function collectGitChangedFiles(cwd: string): Promise<{ gitRoot: string; files: GitChangedFile[] } | null> {
...
function formatChangedFileLine(file: GitChangedFile): string {
...
function parseDiffHunkHeaders(diffText: string): string[] {
...
function extractChangedLinePreview(diffText: string, limit = 8): string[] {
  const lines: string[] = [];
...
function likelyTestPathCandidates(relativePath: string): string[] {
...
function filePathLikelyNeedsTests(relativePath: string): boolean {
...
async function detectLikelyTestCommands(cwd: string): Promise<string[]> {
  ...
  if (
    (await pathExists(path.join(cwd, "pytest.ini"))) ||
...
export const getCwdTool: Tool = {
  ...
  async execute(_input: Record<string, unknown>, context: ToolContext) {
...
export const findSymbolTool: Tool = {
  ...
  async execute(input: Record<string, unknown>, context: ToolContext) {
    ...
    try {
      ...
      return ok(
        this.name,
        matches.map((match) => formatSymbolMatch(context.cwd, match)).join("\n\n"),
        {
          query,
...
export const listExportsTool: Tool = {
  name: "list_exports",
  ...
  async execute(input: Record<string, unknown>, context: ToolContext) {
    ...
    try {
      ...
      return ok(
        this.name,
        exports.map((item) => formatExport(context.cwd, filePath, item)).join("\n\n"),
        {
          path: filePath,
...
export const listImportsTool: Tool = {
  name: "list_imports",
  ...
  async execute(input: Record<string, unknown>, context: ToolContext) {
    ...
    try {
      ...
      return ok(
        this.name,
        imports.map((item) => formatImport(context.cwd, filePath, item)).join("\n\n"),
        {
          path: filePath,
...
export const astEditTool: Tool = {
  ...
  async execute(input: Record<string, unknown>, context: ToolContext) {
    const action = String(input.action ?? "").trim() as AstEditAction;
    const relativePath = String(input.path ?? "").trim();
    ...
    try {
      const result = performAstEdit(workspace, {
        ...
        namedImports: Array.isArray(input.named_imports)
          ? input.named_imports.filter((value): value is string => typeof value === "string")
          : undefined,
...
export const gitChangedFilesTool: Tool = {
  ...
  async execute(input: Record<string, unknown>, context: ToolContext) {
    ...
    return ok(
      this.name,
      files.map((file) => formatChangedFileLine(file)).join("\n"),
      {
        gitRoot: collected.gitRoot,
...
export const gitChangedFileSummariesTool: Tool = {
...
export const estimateTestImpactTool: Tool = {
  ...
  async execute(input: Record<string, unknown>, context: ToolContext) {
    const rawPaths = Array.isArray(input.paths) ? input.paths : [];
    const requestedPaths = rawPaths.filter((value): value is string => typeof value === "string" && value.trim().length > 0);
    ...
    const directTestCandidates = new Set<string>();
    const dependentSources = new Set<string>();
    const dependentTestCandidates = new Set<string>();
...
export const gitRecentCommitsTool: Tool = {
...
export const startCommandSessionTool: Tool = {
...
export const closeCommandSessionTool: Tool = {
